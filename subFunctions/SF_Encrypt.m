...
%Using f function and Xor operation obtain second 16 bit block
enc{4,2}=xor(fun(enc{4,1}),enc{3,2});
%Performing xnor operation on last 16 bit
enc{4,4}=not(xor(K2,enc{3,4}));
%Using f function and Xor operation obtain third 16 bit block
enc{4,3}=xor(fun(enc{4,4}),enc{3,3});
...
% Using f function and swaping tecniques on 16 bit
enc{5,2}=xor(fun(enc{5,1}),enc{4,3});
%performing xnor operation in last 16 bits with K1
enc{5,4}=not(xor(K3,enc{4,4}));
% Using f function and swaping tecniques on 16 bit
enc{5,3}=xor(fun(enc{5,4}),enc{4,2});
%Step no 02
%Swaping Blocks
enc{6,1}=enc{5,2};
enc{6,2}=enc{5,1};
...
%Using f function and Xor operation obtain second 16 bit block
enc{7,2}=xor(fun(enc{7,1}),enc{6,2});
%Performing xnor operation on last 16 bit
enc{7,4}=not(xor(K4,enc{6,4}));
%Using f function and Xor operation obtain third 16 bit block
enc{7,3}=xor(fun(enc{7,4}),enc{6,3});
...
%Using f function and Xor operation obtain third 16 bit block
enc{8,3}=xor(fun(enc{8,4}),enc{7,2});
% Encrypted Cipher
